...
int main()
{
	...
	s.sender_output (v[0], v[1], r.indexlist[receiver_in], r.indexlist[(receiver_in)^0x1], sender_out);
	...
	r.receiver_output (v[receiver_in], receiver_out);
	...
	r.receiver_output (v[0], receiver_out);
	for (int i=0; i<OUTPUT_LENGTH/32; i++)
		printf ("Receiver's output %x  \n", receiver_out[i]);
	printf ("\n\n");
